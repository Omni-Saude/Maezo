"""Shared fixtures for revenue cycle worker tests.

Provides tenant context setup for all revenue cycle v2 worker tests.
This conftest applies to all tests in tests/revenue_cycle/ unless overridden
by domain-specific conftest files (billing, coding, glosa, production).
"""
...
def setup_tenant_context():
    """Automatically set up tenant context for all revenue cycle tests.

    The revenue cycle v2 workers use @require_tenant decorator which validates
