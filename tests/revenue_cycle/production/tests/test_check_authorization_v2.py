...
def test_check_authorization_happy_path_authorized(worker):
    ...
    assert "authorization_results" in result.variables
    assert result.variables["all_authorized"] is True
    assert len(result.variables["authorization_results"]) == 1
    assert result.variables["authorization_results"][0]["status"] == "PROSSEGUIR"
...
def test_check_authorization_mixed_results(worker):
    ...
    assert result.status == TaskStatus.BPMN_ERROR
    assert "authorization_results" in result.variables
    assert "denied_codes" in result.variables
