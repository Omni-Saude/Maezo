...
@pytest.mark.asyncio
async def test_max_attempts_reached(worker, tiss_client, monkeypatch):
    """Test behavior when max attempts already reached (mock failure to test exhaustion)."""
    async def mock_submit_failure(guide_xml, payer_id):
        return TISSSubmissionResult(
            success=False,
            protocol_number=None,
            payer_response_code="ERROR",
            payer_response_message="All previous attempts failed"
        )
...
@pytest.mark.asyncio
async def test_exponential_backoff_calculation(worker, tiss_client, monkeypatch):
...
async def test_worker_metadata(worker):
    ...
    assert worker.TOPIC == "billing.retry_failed_submission"
    assert worker.__class__.__name__ == "RetryFailedSubmissionWorker"
...
@pytest.mark.asyncio
async def test_backoff_cap_at_5_minutes(worker, tiss_client, monkeypatch):
