...
|----------|-------------|-----------|
| **Helm Chart** | `helm/maezo/` | Chart completo com CIB7, Workers, FHIR, Kafka, Redis |
...
- [x] **NOVO**: Pre-commit hook para BPMN compliance — **`scripts/validate/bpmn_pre_commit_hook.sh`**
- [x] **NOVO**: Worker connectivity validator — **`scripts/validate/validate_bpmn_worker_connectivity.py`**
...
- [x] **NOVO**: DMN validation tooling — **`scripts/validate/validate_dmn.py`**
- [x] **NOVO**: DMN inventory generator — **`scripts/generate/dmn_inventory.py`**
- [x] **NOVO**: DMN tenant resolver — **`scripts/generate/dmn_tenant_resolver.py`**
- [x] **NOVO**: Surgery DMN generator — **`scripts/generate/generate_surgery_dmn.py`**
...
|------|---------|-------------------------------|
| **1 — Foundation + Revenue Cycle** | 1–16 | Infra AWS, CI/CD, Docker/K8s, CDC Tasy, FHIR adapters, deploy BPMN/DMN, shadow mode Bradesco |
