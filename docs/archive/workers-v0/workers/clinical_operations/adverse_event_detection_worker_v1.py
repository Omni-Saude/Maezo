...
class DMNAdverseEventClassifier(AdverseEventClassifierProtocol):
    """DMN-backed adverse event classifier using FederatedDMNService."""

    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        self._dmn = dmn_service or get_dmn_service()
        self._logger = get_logger(__name__, component="dmn_adverse")
        self._fallback = AdverseEventClassifierStub()
    ...
    async def classify_event(
        ...
        """Classify adverse event via DMN decision tables."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/adverse_event_classification_001",
                inputs={
            ...
            if result and result.get("classification"):
                return {
                    ...
                    "reasoning": result.get("reasoning", "DMN classification"),
                }
        except (FileNotFoundError, ValueError):
            pass
        except Exception as exc:
            self._logger.warning("dmn_classify_error", error=str(exc))
        return await self._fallback.classify_event(
            event_type, description, severity, factors,
        )
    ...
    async def assess_patient_outcome(
        ...
        """Assess patient outcome via DMN decision tables."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/patient_outcome_assessment_001",
            ...
            self._logger.warning("dmn_outcome_error", error=str(exc))
        return await self._fallback.assess_patient_outcome(
            event_type, severity, patient_ref,
        )
    ...
    async def identify_root_causes(
        ...
        """Identify root causes via DMN decision tables."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="rsk/root_cause/rsk_rca_001",
                inputs={
                    "event_type": event_type,
                    "factor_count": len(factors),
                },
            )
            if result and result.get("root_causes"):
                return {
                    "root_causes": result["root_causes"],
                    "requires_formal_rca": result.get(
                        "requires_formal_rca", False,
                    ),
                }
        except (FileNotFoundError, ValueError):
            pass
        except Exception as exc:
            self._logger.warning("dmn_rca_error", error=str(exc))
        return await self._fallback.identify_root_causes(
            event_type, description, factors,
        )
