...
class DMNCarePlanEngine(CarePlanEngine):
    """DMN-backed care plan engine using FederatedDMNService."""

    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        self._dmn = dmn_service or get_dmn_service()
        self._logger = get_logger(__name__, component="dmn_care_plan")
        self._fallback = StubCarePlanEngine()
    ...
    def generate_plan(
        ...
        """Generate care plan activities via DMN decision tables."""
        ...
        for diag in diagnosis_codes:
            ...
            try:
                result = self._dmn.evaluate(
                    ...
                    table_name="safety/care_plan_activities_001",
                    inputs={
                        "diagnosis_code": code,
                        "goal_count": len(treatment_goals),
                    },
                )
                if result and result.get("results"):
                    for act in result["results"]:
                        activities.append(
                            CareActivity(
                                code=act.get("code", "DMN_ACTIVITY"),
                ...
                self._logger.warning(
                    "dmn_care_plan_error", code=code, error=str(exc),
                )

        if not activities:
            return self._fallback.generate_plan(diagnosis_codes, treatment_goals)
        return activities
...
class CarePlanningWorker:
    ...
    def __init__(
        ...
        self._engine = care_plan_engine or DMNCarePlanEngine()
