...
| Security | Identity | Keycloak | 24 | ADR-008 |
...
```
Keycloak Realm: austa-bpm
├── Clients (client_credentials grant)
│   ├── worker-eligibility     → scope: external-task, fhir-read
│   ├── worker-tiss            → scope: external-task, fhir-read
│   ├── worker-denial          → scope: external-task, fhir-read, fhir-write
│   ├── worker-whatsapp        → scope: external-task
│   ├── worker-clinical        → scope: external-task, fhir-read
│   ├── worker-payment         → scope: external-task
│   ├── cdc-bridge             → scope: process-start, message-correlate
│   └── omnicash-intelligence  → scope: history-read, fhir-read
├── Groups (hospital unit tenants)
│   ├── austa-hospital
│   ├── amh-sp-morumbi
│   ├── amh-rj-barra
│   └── amh-mg-bh
└── Roles
    ...
    ├── operator      → Cockpit read + Tasklist full
    ├── analyst       → Cockpit read + ins7ght
...
- CIB Seven engine deployed (1 replica), Keycloak configured
...
| Java Developer | 1 | 50% Phase 1, 25% Phase 2+ | Spring Boot, CIB Seven config, Keycloak |
