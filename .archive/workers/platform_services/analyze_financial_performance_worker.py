...
class AnalyzeFinancialPerformanceProtocol(ABC):
    ...
    @abstractmethod
    async def analyze_margin_by_specialty(
        self,
        period_start: datetime,
        period_end: datetime,
    ) -> FinancialMetric:
        """
        Analisa margem por especialidade médica.
        ...
        Returns:
            Métrica de margem por especialidade
        """
        pass
...
class AnalyzeFinancialPerformanceStub(AnalyzeFinancialPerformanceProtocol):
    ...
    async def analyze_margin_by_specialty(
        ...
        """Analisa margem por especialidade médica."""
        weighted_margin = 28.5  # %

        return FinancialMetric(
            analysis_type="margin_by_specialty",
            value=weighted_margin,
            unit="%",
            breakdown={
                "Cardiology": 32.1,
                "Orthopedics": 29.8,
                "General Surgery": 24.5,
                "Internal Medicine": 22.3,
            },
        )
...
async def execute(input_data: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        _dmn_config = _dmn.evaluate(
            ...
            inputs={'analysis_types': parsed_input.analysis_types},
        )
    ...
    try:
        ...
        for analysis_type in parsed_input.analysis_types:
            ...
            elif analysis_type == "margin_by_specialty":
                metric = await service.analyze_margin_by_specialty(
                    parsed_input.period_start,
                    parsed_input.period_end,
                )
...
# Topic Kafka
TOPIC = "platform.services.analyze-financial-performance"
