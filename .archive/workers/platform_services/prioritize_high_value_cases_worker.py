...
from healthcare_platform.shared.multi_tenant.context import get_required_tenant, get_current_tenant
    ...
    async def execute(
        ...
        try:
            _dmn_result = self._dmn.evaluate(
                ...
                inputs={'encounter_ids': input_data.encounter_ids},
            )
        ...
        with prioritization_duration_seconds.labels(tenant_id=tenant_id).time():
            try:
                ...
                for encounter_id in input_data.encounter_ids[: input_data.max_cases]:
                    # Extract numeric portion for calculations
                    try:
                        # Try to extract numeric suffix
                        numeric_suffix = ''.join(filter(str.isdigit, encounter_id[-6:]))
                        if not numeric_suffix:
                            numeric_suffix = "1"
                        last_two = numeric_suffix[-2:] if len(numeric_suffix) >= 2 else numeric_suffix
                        last_digit = int(numeric_suffix[-1]) if numeric_suffix else 1
                    ...
                    encounter_data = {
                        ...
                        "estimated_revenue": float(
                            Decimal("1000") + (Decimal(last_two) * 200)
                        ),
                        "estimated_cost": float(
                            Decimal("700") + (Decimal(last_two) * 100)
                        ),
                        "comorbidities": ["HAS", "DM"] if last_digit % 2 == 0 else [],
                        "length_of_stay": last_digit if last_digit > 0 else 1,
                        "icu_admission": last_digit > 7,
                        "procedures": ["PROC1", "PROC2"] if last_digit > 5 else ["PROC1"],
                        "payer": "UNIMED" if last_digit % 3 == 0 else "AMIL",
                        "pending_authorizations": 1 if last_digit % 5 == 0 else 0,
                    }
