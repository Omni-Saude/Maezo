...
Business rules delegated to DMN: clinical_safety/care_planning_coordination.
...
class CarePlanningWorker(BaseExternalTaskWorker):
    """
    Create and update care plans based on diagnosis and treatment goals.

    Responsibilities (thin worker pattern):
    1. Parse input variables
    2. Evaluate DMN for care coordination and planning
    ...
    DMN_DECISION_KEY = "care_planning_coordination"
    DMN_CATEGORY = "clinical_safety"

    def execute(self, context: TaskContext) -> TaskResult:
        """
        Execute CarePlanning operation.
        ...
        try:
            ...
            self.logger.info(
                "Processing CarePlanning operation",
            ...
            self.logger.error(f"CarePlanning operation failed: {e}", exc_info=True)
            return TaskResult.bpmn_error(
                error_code="ERR_CARE_COORDINATION",
                error_message=str(e),
