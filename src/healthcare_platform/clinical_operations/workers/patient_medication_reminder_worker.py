...
Business rules delegated to DMN: clinical_safety/patient_medication_reminder_notification.
...
class PatientMedicationReminderWorker(BaseExternalTaskWorker):
    """
    Patient Medication Reminder Worker
    ...
    TOPIC = "clinical.patient_medication_reminder"
    DMN_DECISION_KEY = "patient_medication_reminder_notification"
    DMN_CATEGORY = "clinical_safety"

    def execute(self, context: TaskContext) -> TaskResult:
        """
        Execute PatientMedicationReminder operation.
        ...
        try:
            ...
            self.logger.info(
                "Processing PatientMedicationReminder operation",
            ...
            self.logger.error(f"PatientMedicationReminder operation failed: {e}", exc_info=True)
