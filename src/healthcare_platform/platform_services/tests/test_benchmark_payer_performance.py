"""Tests for BenchmarkPayerPerformanceStub."""
...
def worker(fhir_client):
    from healthcare_platform.platform_services.workers.benchmark_payer_performance import BenchmarkPayerPerformanceStub
    return BenchmarkPayerPerformanceStub(fhir_client=fhir_client)

@pytest.mark.asyncio
async def test_happy_path_benchmarks_payer_performance(worker, tenant_hospital_a):
    """Should successfully benchmark payer performance."""
    job = {
        "payer_ids": ["PAYER-001", "PAYER-002"],
        "metrics": ["payment_speed", "denial_rate"]
    }
...
async def test_no_tenant_raises(worker):
    ...
    job = {"payer_ids": ["PAYER-001"], "metrics": ["payment_speed"]}
