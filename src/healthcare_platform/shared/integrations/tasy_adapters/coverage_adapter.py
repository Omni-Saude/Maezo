...
class TasyCoverageAdapter(BaseTasyFhirAdapter):
    ...
    async def adapt(self, tasy_data: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        try:
            # Validate required fields — NR_PACIENTE may come from join,
            # NR_ATENDIMENTO can be used as fallback for beneficiary reference
            ...
            coverage: dict[str, Any] = {
                ...
                "identifier": [
                    self._build_identifier(
                        ...
                        value=str(tasy_data.get("NR_SEQ_CONVENIO_PACIENTE", tasy_data.get("CD_CONVENIO", ""))),
                    )
                ],
                "status": self._map_status(tasy_data.get("IE_SITUACAO")),
                "beneficiary": self._build_reference(
                    "Patient",
                    str(tasy_data.get("NR_PACIENTE", tasy_data.get("NR_ATENDIMENTO", ""))),
                ),
                "payor": [
            ...
            if tasy_data.get("NR_CARTEIRA"):
                coverage["subscriberId"] = str(tasy_data["NR_CARTEIRA"])
