"""Liveness, readiness, and metrics HTTP endpoints for FHIR Sync."""
...
_metrics: dict[str, int] = {
    ...
    "fhir_upserts_total": 0,
    "fhir_bundles_total": 0,
    "fhir_deletes_total": 0,
    "events_skipped_no_route": 0,
    "dead_letter_total": 0,
...
class HealthServer:
    ...
    async def _prom_metrics(self, _: web.Request) -> web.Response:
        lines = [
            f"fhir_sync_{k} {v}" for k, v in sorted(_metrics.items())
        ]
        return web.Response(
            text="\n".join(lines) + "\n",
