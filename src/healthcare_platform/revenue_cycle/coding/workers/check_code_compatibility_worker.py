"""Check CID-10 / TUSS code compatibility - thin worker.
...
class CheckCodeCompatibilityWorker:
    """Thin worker: delegates compatibility checks to DMN tables. ORPHAN: No companion DMN tables exist yet."""
    ...
    async def _execute_impl(self, task_variables: dict[str, Any]) -> dict[str, Any]:
        ...
        self._logger.info("compatibility_started", encounter_id=enc_id, cid10_count=len(cid10), tuss_count=len(tuss), tenant_id=ctx.tenant_id)
        ...
        output = CheckCodeCompatibilityOutput(compatible=compat, incompatibilities=incomps, warnings=warns)

        if not compat:
            self._logger.error("incompatible_codes", encounter_id=enc_id, incompatibility_count=len(incomps), tenant_id=ctx.tenant_id)
        ...
        if warns:
            self._logger.warning("compatibility_warnings", encounter_id=enc_id, warning_count=len(warns), tenant_id=ctx.tenant_id)

        self._logger.info("compatibility_passed", encounter_id=enc_id, warning_count=len(warns), tenant_id=ctx.tenant_id)
