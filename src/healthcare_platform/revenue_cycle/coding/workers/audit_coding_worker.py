"""Quality audit of clinical coding by supervisor - thin worker.
...
class AuditCodingWorker:
    """Thin worker: delegates audit logic to DMN tables. ORPHAN: No companion DMN tables exist yet."""
    ...
    async def _execute_impl(self, task_variables: dict[str, Any]) -> dict[str, Any]:
        ...
        self._logger.info("audit_started", encounter_id=enc_id, coded_by=coded_by, cid10_count=len(cid10), tuss_count=len(tuss), tenant_id=ctx.tenant_id)
        ...
        output = AuditCodingOutput(auditScore=score, auditFindings=findings, auditRecommendation=rec, requiresRevision=req_rev)

        if score < self._FAIL_THRESHOLD:
            self._logger.error("audit_failed", encounter_id=enc_id, score=score, recommendation=rec, tenant_id=ctx.tenant_id)
        ...
        self._logger.info("audit_completed", encounter_id=enc_id, score=score, recommendation=rec, tenant_id=ctx.tenant_id)
