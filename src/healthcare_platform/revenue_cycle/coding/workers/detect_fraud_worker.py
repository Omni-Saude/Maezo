"""Detect coding fraud patterns - thin worker.
...
class DetectFraudWorker(BaseExternalTaskWorker):
    ...
    async def _execute_impl(self, task_variables: dict[str, Any]) -> dict[str, Any]:
        ...
        self._logger.info("fraud_detection_started", encounter_id=inp.encounter_id, cid10_count=len(inp.validated_cid10), tuss_count=len(inp.validated_tuss), tenant_id=ctx.tenant_id)
        ...
        if risk > 80:
            self._logger.warning("fraud_detected_high_risk", encounter_id=inp.encounter_id, risk_score=risk, tenant_id=ctx.tenant_id)
        ...
        self._logger.info("fraud_detection_completed", encounter_id=inp.encounter_id, risk_score=risk, alert_count=len(alerts), tenant_id=ctx.tenant_id)
...
def register_worker(dmn_service: FederatedDMNService | None = None) -> DetectFraudWorker:
    return DetectFraudWorker(dmn_service=dmn_service)
