"""Extract clinical data from FHIR encounter for coding - thin worker.
...
class ExtractClinicalDataWorker(BaseExternalTaskWorker):
    """Thin worker: delegates extraction to ClinicalDataExtractionService + DMN."""
    ...
    async def _execute_impl(self, task_variables: dict[str, Any]) -> dict[str, Any]:
        ...
        self._logger.info("extract_started", encounter_id=enc_id, tenant_id=ctx.tenant_id)
        result = await self.service.extract_with_dmn(enc_id, ctx.tenant_id, self.dmn_service)
        self._logger.info("extract_completed", encounter_id=enc_id, tenant_id=ctx.tenant_id)
        return result
...
def register_worker(fhir_client: FHIRClientProtocol) -> ExtractClinicalDataWorker:
    return ExtractClinicalDataWorker(fhir_client=fhir_client)
