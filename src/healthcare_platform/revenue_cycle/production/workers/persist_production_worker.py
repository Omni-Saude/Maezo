...
class PersistProductionWorker(BaseExternalTaskWorker):
    ...
    def execute(self, context: TaskContext) -> TaskResult:
        try:
            ...
            breakdown = variables.get("breakdown", [])
            encounter = variables.get("encounter", "")
            value = variables.get("value", "0.00")
            patient_ref = variables.get("patientReference", variables.get("patient_reference", ""))
            production_id = str(uuid4())

            breakdown_procedures = breakdown if isinstance(breakdown, list) else []
            encounter_ref = encounter
            total_amount = str(value)

            dmn_result = self.evaluate_dmn(context=context, decision_key=self.DMN_DECISION_KEY,
                variables={"procedureCount": len(breakdown_procedures), "totalAmount": value,
            ...
            self.logger.info(f"Persisting production: {len(breakdown_procedures)} procedures, total={total_amount}",
                extra={"tenant_id": context.tenant_id, "production_id": production_id})

            result = self.service.persist(breakdown_procedures, encounter_ref, patient_ref, total_amount, production_id)

            if resultado == "REVISAR":
                result["requiresReview"] = True
                result["charge_item_references"] = []

            result["productionId"] = production_id
