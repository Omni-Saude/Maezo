...
class AssignPricesWorker(BaseExternalTaskWorker):
    ...
    def execute(self, context: TaskContext) -> TaskResult:
        try:
            ...
            payer = variables.get("payer", "tuss_default")
            ...
            self.logger.info(f"Assigning prices: {len(procedures)} procedures",
                extra={"tenant_id": context.tenant_id, "contract_id": contract})

            result = self.service.assign_prices(procedures, contract, payer or "tuss_default")
            ...
            return TaskResult.success({"pricedProcedures": result.get("priced_procedures", [])})
