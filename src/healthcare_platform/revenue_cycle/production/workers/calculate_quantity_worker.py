...
class CalculateQuantityWorker(BaseExternalTaskWorker):
    ...
    def execute(self, context: TaskContext) -> TaskResult:
        ...
        try:
            ...
            procedures = variables.get("procedures") or variables.get("captured_procedures", [])
            # Handle JSON string
            if isinstance(procedures, str):
                try:
                    ...
                    procedures = []
            if not procedures:
                procedures = [{"code": variables.get("procedureCode", "10101012"), "quantity": 1}]
            ...
            return TaskResult.success({
                "procedureQuantities": [p.get("quantity", 1) for p in quantified],
                "measurementUnits": [p.get("quantity_method", "unit") for p in quantified],
                "breakdown": quantified,
                "totalValue": total_items,
            })

        except Exception as e:
            self.logger.error(f"Quantity calculation failed, using defaults: {e}", exc_info=True)
            return TaskResult.success({
                "procedureQuantities": [1],
