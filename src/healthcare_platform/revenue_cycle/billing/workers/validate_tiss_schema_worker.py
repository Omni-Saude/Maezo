...
class ValidateTISSSchemaWorker(BaseExternalTaskWorker):
    ...
    async def execute(self, context: Union[TaskContext, types.SimpleNamespace]) -> ProcessTaskResult:
        ...
        try:
            ...
            tiss_xml = variables.get("tissXml", variables.get("tiss_xml", ""))
            ...
            if resultado == "BLOQUEAR":
                ...
                return ProcessTaskResult(success=True, variables={
                    **validation_result,
                    "schemaValid": validation_result.get("schema_valid", False),
                    "schemaErrors": validation_result.get("schema_errors", []),
                })
