...
class StubAvailabilityChecker(AvailabilityChecker):
    ...
    async def check_availability(
        ...
        """Check availability using DMN timing rules."""
        from datetime import timedelta

        # Try DMN evaluation for timing constraints
        try:
            result = self.dmn_service.evaluate(
                tenant_id=tenant_id or get_required_tenant(),
                category='authorization',
                table_name='auth_timing_001',
                inputs={
            ...
            # DMN could return constraints like working_hours, break_times, etc.
            work_start_hour = result.get('work_start_hour', 9)
            work_end_hour = result.get('work_end_hour', 17)
        ...
        slots = []
        current = start_date.replace(hour=work_start_hour, minute=0, second=0, microsecond=0)
        end = end_date.replace(hour=work_end_hour, minute=0, second=0, microsecond=0)
