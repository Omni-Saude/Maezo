...
def worker(fhir_client):
    from healthcare_platform.platform_services.workers.optimize_pricing_strategy import OptimizePricingStrategyStub
    return OptimizePricingStrategyStub(fhir_client=fhir_client)

@pytest.mark.asyncio
async def test_happy_path_optimizes_pricing_strategy(worker, tenant_hospital_a):
