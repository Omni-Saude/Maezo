...
class PatientCopayEstimateWorker(BaseExternalTaskWorker):
    ...
    def execute(self, context: TaskContext) -> TaskResult:
        ...
        try:
            ...
            appointment_id = variables.get("appointment_id", "")
            estimated_copay = float(variables.get("estimated_copay", 0))
            insurance_coverage = float(variables.get("insurance_coverage", 0))
            appointment_date = variables.get("appointment_date", "")

            if not patient_id or not phone_number or not appointment_id:
                return TaskResult.bpmn_error(
                    error_code="ERR_INVALID_INPUT",
                    error_message="Missing patient_id, phone_number, or appointment_id",
                )
            ...
            return TaskResult.success({
                ...
                "acao": acao,
                "risco": risco,
                "formatted_copay": formatted_copay,
            })
