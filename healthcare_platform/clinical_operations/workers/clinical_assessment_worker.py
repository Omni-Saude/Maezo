...
Business rules delegated to DMN: clinical_safety/clinical_assessment_assessment.
...
class ClinicalAssessmentWorker(BaseExternalTaskWorker):
    """
    Perform initial clinical assessment and triage using Manchester Protocol.
    ...
    TOPIC = "clinical.clinical_assessment"
    DMN_DECISION_KEY = "clinical_assessment_assessment"
    DMN_CATEGORY = "clinical_safety"

    def execute(self, context: TaskContext) -> TaskResult:
        """
        Execute ClinicalAssessment operation.
        ...
        try:
            ...
            self.logger.info(
                "Processing ClinicalAssessment operation",
            ...
            self.logger.error(f"ClinicalAssessment operation failed: {e}", exc_info=True)
            return TaskResult.bpmn_error(
                error_code="ERR_CLINICAL_PROCESSING",
                error_message=str(e),
