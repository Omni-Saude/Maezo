...
class DMNDrugInteractionChecker(DrugInteractionChecker):
    """DMN-backed drug interaction checker using FederatedDMNService."""
    ...
    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        ...
        self._logger = get_logger(__name__, component="dmn_ddi")
        self._fallback = StubDrugInteractionChecker()

    def check_interactions(
        self,
        medications: list[MedicationOrder],
    ) -> list[MedicationInteraction]:
        """Check for drug-drug interactions via DMN tables."""
        interactions: list[MedicationInteraction] = []
        tenant_id = get_required_tenant().tenant_id

        for i, med1 in enumerate(medications):
            for med2 in medications[i + 1:]:
                name1 = med1.medication_name.upper().strip()
                name2 = med2.medication_name.upper().strip()
                ...
                for subcat in self._DDI_SUBCATEGORIES:
                    try:
                        result = self._dmn.evaluate(
                            ...
                            inputs={"drug_1": name1, "drug_2": name2},
                        )
                        if result:
                            interactions.append(
                                MedicationInteraction(
        ...
        if not interactions:
            return self._fallback.check_interactions(medications)
        return interactions


class DMNAllergyChecker(AllergyChecker):
    """DMN-backed allergy checker using FederatedDMNService."""

    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        self._dmn = dmn_service or get_dmn_service()
        self._logger = get_logger(__name__, component="dmn_allergy")
        self._fallback = StubAllergyChecker()
    ...
    def check_allergies(
        ...
        """Check medications against patient allergies via DMN tables."""
        warnings: list[str] = []
        tenant_id = get_required_tenant().tenant_id
            ...
            for medication in medications:
                try:
                    result = self._dmn.evaluate(
                        ...
                        inputs={
                            "allergy": allergy.upper().strip(),
                            "medication": medication.medication_name.upper().strip(),
                        },
                    )
                    if result and result.get("has_reaction"):
                        warnings.append(
                            _(
                                "ALERTA: {medication} pode causar reação "
                                "(alergia a {allergy}): {detail}"
                            ).format(
                    ...
                    self._logger.warning(
                        "dmn_allergy_check_error", error=str(exc),
                    )

        if not warnings:
            return self._fallback.check_allergies(medications, allergies)
        return warnings
...
class MedicationManagementWorker:
    ...
    def __init__(
        ...
        self._interaction_checker = interaction_checker or DMNDrugInteractionChecker()
        self._allergy_checker = allergy_checker or DMNAllergyChecker()
