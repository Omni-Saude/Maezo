...
class DMNClinicalAuditEngine(ClinicalAuditEngineProtocol):
    """DMN-backed clinical audit engine using FederatedDMNService."""

    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        self._dmn = dmn_service or get_dmn_service()
        self._logger = get_logger(__name__, component="dmn_audit")
        self._fallback = ClinicalAuditEngineStub()

    async def audit_documentation(
        self, encounter_ref: str, period: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Audit documentation via DMN rules."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/audit_documentation_001",
                inputs={"encounter_ref": encounter_ref, "period": period or ""},
            )
            if result and result.get("compliance_score") is not None:
                return result
        except (FileNotFoundError, ValueError):
            pass
        except Exception as exc:
            self._logger.warning("dmn_audit_doc_error", error=str(exc))
        return await self._fallback.audit_documentation(encounter_ref, period)

    async def audit_medication_records(
        self, encounter_ref: str, period: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Audit medication records via DMN rules."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/audit_medication_001",
                inputs={"encounter_ref": encounter_ref, "period": period or ""},
            )
            if result and result.get("compliance_score") is not None:
                return result
            ...
            self._logger.warning("dmn_audit_med_error", error=str(exc))
        return await self._fallback.audit_medication_records(encounter_ref, period)

    async def audit_procedures(
        self, encounter_ref: str, period: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Audit procedures via DMN rules."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/audit_procedures_001",
                inputs={"encounter_ref": encounter_ref, "period": period or ""},
            )
            if result and result.get("compliance_score") is not None:
                return result
            ...
            self._logger.warning("dmn_audit_proc_error", error=str(exc))
        return await self._fallback.audit_procedures(encounter_ref, period)

    async def audit_protocol_adherence(
        self, encounter_ref: str, period: Optional[str] = None,
    ) -> Dict[str, Any]:
        """Audit protocol adherence via DMN rules."""
        tenant_id = get_required_tenant().tenant_id
        try:
            result = self._dmn.evaluate(
                tenant_id=tenant_id,
                category="clinical_safety",
                table_name="safety/audit_protocol_adherence_001",
                inputs={"encounter_ref": encounter_ref, "period": period or ""},
            )
            if result and result.get("compliance_score") is not None:
                return result
            ...
            self._logger.warning("dmn_audit_protocol_error", error=str(exc))
        return await self._fallback.audit_protocol_adherence(encounter_ref, period)
...
class ClinicalAuditingWorker:
    ...
    def __init__(
        ...
        self.audit_engine = audit_engine or DMNClinicalAuditEngine()
