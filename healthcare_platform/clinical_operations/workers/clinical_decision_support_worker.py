...
class DMNDecisionSupportEngine(DecisionSupportEngineProtocol):
    """DMN-backed decision support engine using FederatedDMNService."""

    _DDI_SUBCATEGORIES = (
        "bleed", "hepato", "nephro", "qt",
        "major", "moderate", "contraind", "serotonin",
    )

    def __init__(self, dmn_service: FederatedDMNService | None = None) -> None:
        self._dmn = dmn_service or get_dmn_service()
        self._logger = get_logger(__name__, component="dmn_cds")
        self._fallback = DecisionSupportEngineStub()
    ...
    async def check_drug_interactions(
        ...
        """Check drug interactions via DDI DMN tables."""
        tenant_id = get_required_tenant().tenant_id
        alerts: List[Dict[str, Any]] = []
        for i, med1 in enumerate(medications):
            for med2 in medications[i + 1:]:
                n1 = med1.get("name", "").upper()
                n2 = med2.get("name", "").upper()
                for subcat in self._DDI_SUBCATEGORIES:
                    try:
                        result = self._dmn.evaluate(
                            tenant_id=tenant_id,
                            category="clinical_safety",
                            table_name=f"ddi/{subcat}/ddi_{subcat}_001",
                            inputs={"drug_1": n1, "drug_2": n2},
                        )
                        ...
                        continue
        if not alerts:
            return await self._fallback.check_drug_interactions(
    ...
    async def check_allergies(
        ...
        """Check allergy contraindications via DMN."""
        tenant_id = get_required_tenant().tenant_id
        alerts: List[Dict[str, Any]] = []
            ...
            for med in medications:
                try:
                    result = self._dmn.evaluate(
                        tenant_id=tenant_id,
                        category="clinical_safety",
                        table_name="allergy/allergy_cross_reaction_001",
                        inputs={
                            "allergy": allergy.upper(),
                            "medication": med.get("name", "").upper(),
                        },
                    )
                    if result and result.get("has_reaction"):
                        alerts.append({
                            "type": "allergy",
                            "severity": "CRITICAL",
                    ...
                    continue
        if not alerts:
            return await self._fallback.check_allergies(medications, allergies)
        return alerts
    ...
    async def check_contraindications(
        ...
        """Check contraindications via DMN."""
        tenant_id = get_required_tenant().tenant_id
        alerts: List[Dict[str, Any]] = []
        for med in medications:
            try:
                ...
                if patient_context.renal_function:
                    inputs["renal_function"] = patient_context.renal_function
                if patient_context.hepatic_function:
                    inputs["hepatic_function"] = patient_context.hepatic_function
                if patient_context.pregnancy_status is not None:
                    inputs["pregnant"] = patient_context.pregnancy_status
                result = self._dmn.evaluate(
                    tenant_id=tenant_id,
                    category="clinical_safety",
                    table_name="dli/renal/dli_renal_001",
                    inputs=inputs,
                )
                if result and result.get("contraindicated"):
                    alerts.append({
                ...
                continue
        if not alerts:
            return await self._fallback.check_contraindications(
                medications, patient_context,
            )
        return alerts
    ...
    async def generate_treatment_recommendations(
        ...
        """Generate treatment recommendations via DMN."""
        tenant_id = get_required_tenant().tenant_id
        ...
        for diag in diagnoses:
            try:
                result = self._dmn.evaluate(
                    tenant_id=tenant_id,
                    category="clinical_safety",
                    table_name="safety/treatment_recommendation_001",
                    inputs={"diagnosis_code": diag},
                )
                ...
                continue
        if not recs:
            return await self._fallback.generate_treatment_recommendations(
                diagnoses, patient_context,
            )
        return recs
    ...
    async def assess_diagnostic_support(
        ...
        """Delegate to fallback -- diagnostic reasoning beyond DMN scope."""
        return await self._fallback.assess_diagnostic_support(
            symptoms, lab_results, patient_context,
        )
