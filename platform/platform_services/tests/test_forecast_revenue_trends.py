...
def worker(fhir_client):
    from platform.platform_services.workers.forecast_revenue_trends import ForecastRevenueTrendsStub
    return ForecastRevenueTrendsStub(fhir_client=fhir_client)

@pytest.mark.asyncio
async def test_happy_path_forecasts_revenue_trends(worker, tenant_austa):
