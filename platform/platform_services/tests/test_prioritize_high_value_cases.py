...
def worker(fhir_client):
    from platform.platform_services.workers.prioritize_high_value_cases import PrioritizeHighValueCasesStub
    return PrioritizeHighValueCasesStub(fhir_client=fhir_client)

@pytest.mark.asyncio
async def test_happy_path_prioritizes_high_value_cases(worker, tenant_austa):
