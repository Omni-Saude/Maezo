...
@require_tenant
@track_task_execution(worker_topic="clinical.care_team")
async def execute(task_variables: dict[str, Any]) -> dict[str, Any]:
    ...
    from platform.shared.integrations.fhir_client import FHIRClientStub
    fhir_client = FHIRClientStub()

    # Create service
    service = CareTeamCoordinationStub(fhir_client=fhir_client)
